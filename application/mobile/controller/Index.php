...
class Index extends MobileBase {
    ...
    public function index(){
      ...
      //  $this->assign('hot_goods',$hot_goods);
        /**商品精选**/
        $new_goods = M('goods')->field('goods_id,original_img,shop_price,market_price,goods_name,sales_sum')->where("is_new=1 and is_on_sale=1")->order('sort asc')->limit(40)->cache(true,TPSHOP_CACHE_TIME)->select();//
    ...
    public function test_order(){
        update_pay_status('201910051704005025');
    }
    ...
    public function get_integral(){
         //0撸的加入到积分去
          ...
          if($order_id){
              ...
              //说明是订单广告，直接按订单百分一
              $orderInfo = M('order')->field('order_amount,user_money,order_id,total_amount')->where('order_id='.$order_id)->find();
             //订单广告收益
              $orderAdvProfit = ($orderInfo['total_amount']) * ($orderConf/100);
