...
/*     function test(){
    ...
    function get_money(){
       ...
       // dump($orderInfo);
        ...
        if($day_mark == 'fifteen'){//如果为15-28天的话查询当天有没有订单生成
            ...
            $order =  M('order')->field('order_id,order_amount,user_money,pay_time,total_amount,integral_money')->where('user_id = '.$this->user_id .' and pay_time >'.$orderInfo['can_receive'].' and order_status not in(3) and pay_time<'.$end_time.'  and  pay_status = 1 and type = 1 and matching_order_id = 0')
    ...
    function time_days($end_time){
        ...
        $second = $end_time - $today; //结束时间戳减去当前时间戳
...
//            header("Location: " . U('Mobile/User/index'));
    ...
    /*         //是否开启注册验证码机制
           ...
           // $invite = I('invite');
            $invite = session('pid');
            if(!empty($invite)){
                $invite = get_user_info($invite,0);//根据手机号查找邀请人
                if(empty($invite)){
            ...
            $this->getWchatQrcode($data['result']['user_id']);
    ...
    public function wx_pic(){
        $pic = "http://thirdwx.qlogo.cn/mmopen/vi_32/Q0j4TwGTfTKYDYYyAZQJrCs8viaE6PVpfjXwCsQ5OiaO2pkYS179gDrvVQuTiagibUEeXP4NUH1mBaJyUgFzMABrGQ/132";
        $rs = $this->curl_file_get_contents($pic);
        dump($rs);
    ...
    function getWchatQrcode($users_id){
        ...
        $object = new \QRcode();
        $ad = 'code/code_jpg/'.$users_id.'.jpg';
        $object->png($url, $ad, $errorCorrectionLevel, $matrixPointSize, 2);
        /*   $logo = 'http://yuewanji.hlz4319.com/public/static/yuwanji/img/head_pic.jpg';//准备好的logo图片
        $QR = 'code/'.$users_id.'.jpg';//已经生成的原始二维码图
        ...
        //输出图片  带logo图片
        imagepng($QR, 'code/'.$users_id.'.png'); */
        $this->share_img($users_id);
    }
    //合成图片
    function share_img($users_id){
        ...
        //$image = 'code/'.date("YmdHis").rand(100000,999999).".".$hz;
        $image = 'code/'.$users_id.".".$hz;
        //创建图片的实例
        ...
        M('users')->where('user_id',$users_id)->save(array('code_img'=>'/'.$image));
        imagedestroy($dst);
        imagedestroy($src);
        return $image;
...
//        }
    ...
    function receive_limit($user_id = 0)
    {
        ...
        try{
            ...
            $profit_cons_beans = Db::name('config')->where(['name' => 'profit_cons_beans'])->value('value');
            $beans_number = ceil($money * ($profit_cons_beans/100));
            if($have_beans < $beans_number){
                return json([
                    'status'  =>  -1,
                    'msg'   =>  '悦玩豆不足，请到交易大厅购买',
                    'data'  =>  [],
            ...
            Db::name('users')->where(['user_id' => $user_id])->setDec('happy_beans',$beans_number);
            Db::name('adv_log')->insert([
                'user_id'   => $user_id,
                'user_money' => $beans_number,
                'add_time'  =>  time(),
                'desc'      =>  '领取分享收益消耗悦玩豆',
                'type'      =>  1
            ]);
