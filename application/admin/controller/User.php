...
class User extends Base
{
    ...
    public function pubtaskHandle()
    {
        $data = I('post.');
        $data['publish_time'] = strtotime($data['publish_time']);
        $result = $this->validate($data, 'Pubtask.'.$data['act'], [], true);
        if ($result !== true) {
            $this->ajaxReturn(['status' => 0, 'msg' => '参数错误', 'result' => $result]);
        }
        $data['act'] = $data['act']?$data['act']:'add';
        $task['desc'] = $data['title'];
        $task['add_time'] = time();
        $task['is_open'] = $data['is_open'];
        $task['content'] = $data['content'];
        $task['description'] = $data['description'];//任务文案
        $task['type'] = $data['type'];
        ...
        if ($data['act'] == 'add') {
            ...
            $r = M('task')->where("id",$data['task_id'])->save($task);
        }elseif ($data['act'] == 'del') {
    ...
    public function pubtask(){
        ...
        if($data['id']){
            ...
            $info = M('task')->where('id',$data['id'])->find();
        }else if($data['type']){
    ...
    public function tasklist(){
        $tasklist = M('task')->order('id desc')->select();
